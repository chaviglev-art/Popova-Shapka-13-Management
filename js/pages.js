...
/* ================= HOME ================= */
PAGES.home = () => canManage() ? homeAdmin() : homeResident();
function homeAdmin() {
...
/* ================= FINANCES ================= */
PAGES.finances = () => {
  const tabs = canManage() ? ['overview', 'matrix', 'expenses', 'history'] : ['overview', 'statement', 'matrix', 'expenses'];
  ...
  const actions = canManage() ? `<button class="btn btn-secondary" onclick="openExpenseModal()">${icon('receipt')}${t('add_expense')}</button><button class="btn btn-primary" onclick="openPaymentModal()">${icon('plus')}${t('record_payment')}</button>` : `<button class="btn btn-secondary" onclick="window.print()">${icon('print')}${t('print')}</button>`;
  return head(t('fin_title'), canManage() ? t('fin_sub_admin') : t('fin_sub_res'), actions) + tabBar + ({ overview: finOverview, matrix: finMatrix, statement: finStatement, expenses: finExpenses, history: finHistory })[ui.finTab]();
};
function finOverview() {
  ...
  <div class="card" style="margin-top:18px"><div class="card-head"><h3>${t('status')} · ${fmtPeriod(per)}</h3></div><div class="table-wrap"><table><thead><tr><th>${t('unit')}</th><th>${t('owner')}</th><th class="num">${t('fee')}</th><th class="num">${t('paid')}</th><th class="num">${t('balance')}</th><th>${t('status')}</th></tr></thead><tbody>
    ${DB.units.map(u => { const p = paidFor(u.id, per); const b = unitBalance(u.id); return `<tr class="${u.id === currentUser.unitId ? 'me' : ''}" ${canManage() ? `style="cursor:pointer" onclick="openUnitDetail('${u.id}')"` : ''}><td><b>${unitLabel(u)}</b></td><td>${esc(u.owner)}</td><td class="num">${fmtMoney(u.fee, { noDual: true })}</td><td class="num">${fmtMoney(p, { noDual: true })}</td><td class="num" style="color:${b < 0 ? 'var(--bad)' : 'var(--good)'};font-weight:600">${fmtMoney(b, { noDual: true })}</td><td>${payBadge(payStatus(u, per))}</td></tr>`; }).join('')}
  </tbody></table></div></div>`;
}
function finMatrix() {
...
/* ================= REQUESTS ================= */
function requestRow(r) { const u = DB.units.find(x => x.id === r.unitId); return `<div class="req" onclick="openRequestDetail('${r.id}')"><span class="pr ${r.priority}" title="${t('prio_' + r.priority)}"></span><div style="min-width:0"><div class="row wrap" style="gap:6px"><b>${esc(r.subject)}</b>${statusBadge(r.status)}</div><div class="small muted">${canManage() ? unitLabel(u) + ' · ' : ''}${t('cat_' + r.category)} · ${fmtDate(r.date)}${r.comments && r.comments.length ? ' · 💬 ' + r.comments.length : ''}${r.photo ? ' · 📷' : ''}</div></div>${icon('chevron_r', 'subtle')}</div>`; }
PAGES.requests = () => {
  let list = canManage() ? DB.requests.slice() : DB.requests.filter(r => r.unitId === currentUser.unitId);
  ...
  return head(canManage() ? t('inbox_title') : t('req_title'), canManage() ? t('inbox_sub') : t('req_sub'), `<button class="btn btn-primary" onclick="openRequestModal()">${icon('plus')}${t('req_new')}</button>`) +
...
function openRequestModal() {
  const u = myUnit();
  openModal({ title: t('req_new'), body: `${canManage() ? `<div class="field"><label>${t('unit')}</label><select class="select" id="rqUnit">${DB.units.map(x => `<option value="${x.id}">${unitLabel(x)} — ${esc(x.owner)}</option>`).join('')}</select></div>` : `<p class="small muted" style="margin-bottom:12px">${unitLabel(u)} · ${esc(u.owner)}</p>`}
...
function openRequestDetail(id) {
  ...
  const thread = `<div class="thread">${(r.comments || []).map(c => `<div class="msg ${c.by === 'admin' ? 'admin' : ''}"><div class="who">${c.by === 'admin' ? t('role_manager') : esc(u ? u.owner : '')} · ${fmtDate(c.date)}</div>${esc(c.text)}</div>`).join('')}</div>`;
  openModal({ title: esc(r.subject), wide: true, body: `<div class="row wrap" style="gap:6px;margin-bottom:12px">${statusBadge(r.status)}<span class="badge b-neutral">${t('cat_' + r.category)}</span><span class="badge b-neutral"><span class="pr ${r.priority}" style="width:8px;height:8px;margin:0"></span>${t('prio_' + r.priority)}</span><span class="small muted right">${unitLabel(u)} · ${esc(u ? u.owner : '')} · ${fmtDate(r.date)}</span></div>
    ...
    <div class="field" style="margin-top:14px"><label>${canManage() ? t('reply') : t('add_comment')}</label><textarea class="textarea" id="rdText" rows="3"></textarea></div>
    ${canManage() ? `<div class="grid-2"><div class="field"><label>${t('status')}</label><select class="select" id="rdStatus">${selectOpts(['new', 'in_progress', 'resolved', 'closed'], 'st_', r.status)}</select></div><div class="field"><label>${t('internal_note')}</label><input class="input" id="rdNote" value="${esc(r.internalNote)}"></div></div>` : ''}`,
    footer: `${canManage() ? `<button class="btn btn-danger btn-sm" onclick="confirmDialog('',()=>{DB.requests=DB.requests.filter(x=>x.id!=='${id}');audit('request_deleted');saveData();closeModal();render()})">${icon('trash')}${t('delete')}</button>` : ''}<span class="grow"></span><button class="btn btn-secondary" onclick="closeModal()">${t('close')}</button><button class="btn btn-primary" id="rdSave">${icon('check')}${t('save')}</button>`,
    onOpen: ov => $('#rdSave', ov).onclick = () => { const txt = $('#rdText').value.trim(); if (txt) r.comments.push({ by: canManage() ? 'admin' : 'owner', date: todayISO(), text: txt }); if (canManage()) { const ns = $('#rdStatus').value; if (ns !== r.status && (ns === 'resolved' || ns === 'closed')) r.resolvedAt = todayISO(); r.status = ns; r.internalNote = $('#rdNote').value.trim(); } audit('request_updated', r.subject); saveData(); closeModal(); toast(t('saved')); render(); } });
}
...
PAGES.votes = () => {
  ...
  return head(t('votes_title'), t('votes_sub'), canManage() ? `<button class="btn btn-primary" onclick="openVoteModal()">${icon('plus')}${t('new_vote')}</button>` : '') + (votes.length ? votes.map(voteCard).join('') : `<div class="card">${empty('vote')}</div>`);
};
function voteCard(v) {
  ...
  // A manager is still a unit owner and should still get their own vote, same as any
  // other resident — myUnit() alone is the right gate (the true admin has no unit at
  // all, so this naturally excludes them without needing a role check here).
  const canVote = !over && !my && myUnit(); const turnout = Math.round(total / DB.units.length * 100); const counts = {}; Object.values(ballots).forEach(o => counts[o] = (counts[o] || 0) + 1);
  return `<div class="card" style="margin-bottom:14px"><div class="card-body">
    <div class="row wrap" style="align-items:flex-start;margin-bottom:6px"><h3 style="font-size:1.05rem" class="grow">${esc(v.title)}</h3>${over ? `<span class="badge b-neutral">${t('vote_closed')}</span>` : `<span class="badge b-good">${t('vote_active')}</span>`}${canManage() ? `${!v.closed ? `<button class="btn btn-secondary btn-sm" onclick="const v=DB.votes.find(x=>x.id==='${v.id}');v.closed=true;audit('vote_closed');saveData();render()">${t('close_vote')}</button>` : ''}<button class="btn btn-ghost btn-sm btn-icon" onclick="confirmDialog('',()=>{DB.votes=DB.votes.filter(x=>x.id!=='${v.id}');audit('vote_deleted');saveData();render()})">${icon('trash')}</button>` : ''}</div>
...
PAGES.news = () => {
  ...
  return head(t('news_title'), t('news_sub'), canManage() ? `<button class="btn btn-secondary" onclick="openWorkModal()">${icon('wrench')}${t('add_work')}</button><button class="btn btn-primary" onclick="openNewsModal()">${icon('plus')}${t('post_news')}</button>` : '') + bannerHtml() +
    `<div class="two-col"><div class="stack">${news.length ? news.map(n => `<div class="card news-card ${n.pinned ? 'pinned' : ''}"><div class="row wrap tiny subtle">${fmtDate(n.date, 'long')}${n.pinned ? `<span class="badge b-info">${icon('pin')}${t('pinned')}</span>` : ''}${canManage() ? `<span class="right row" style="gap:2px"><button class="btn btn-ghost btn-sm btn-icon" onclick="openNewsModal('${n.id}')">${icon('edit')}</button><button class="btn btn-ghost btn-sm btn-icon" onclick="confirmDialog('',()=>{DB.news=DB.news.filter(x=>x.id!=='${n.id}');audit('news_deleted');saveData();render()})">${icon('trash')}</button></span>` : ''}</div><h3>${esc(n.title)}</h3><p>${esc(n.body)}</p></div>`).join('') : `<div class="card">${empty('news')}</div>`}</div>
    <div class="stack"><div class="card"><div class="card-head"><h3>${t('works_title')}</h3></div>${DB.works.length ? `<div class="list">${DB.works.map(w => `<div class="list-item" style="align-items:flex-start"><div class="grow"><div class="row wrap"><b>${esc(w.title)}</b>${statusBadge(w.status, 'work')}</div>${w.desc ? `<div class="small muted">${esc(w.desc)}</div>` : ''}<div class="progress" style="margin:8px 0 4px"><i style="width:${w.progress}%"></i></div><div class="tiny subtle">${w.start ? fmtDate(w.start) : ''}${w.end ? ' → ' + fmtDate(w.end) : ''} · ${w.progress}%</div></div>${canManage() ? `<div class="row" style="gap:2px"><button class="btn btn-ghost btn-sm btn-icon" onclick="openWorkModal('${w.id}')">${icon('edit')}</button><button class="btn btn-ghost btn-sm btn-icon" onclick="confirmDialog('',()=>{DB.works=DB.works.filter(x=>x.id!=='${w.id}');saveData();render()})">${icon('trash')}</button></div>` : ''}</div>`).join('')}</div>` : empty('wrench')}</div></div></div>`;
};
...
PAGES.calendar = () => {
  ...
  return head(t('cal_title'), t('cal_sub'), canManage() ? `<button class="btn btn-primary" onclick="openEventModal()">${icon('plus')}${t('add_event')}</button>` : '') +
...
PAGES.documents = () => {
  ...
  return head(t('docs_title'), t('docs_sub'), canManage() ? `<button class="btn btn-primary" onclick="openDocModal()">${icon('plus')}${t('add_doc')}</button>` : '') + (groups.length ? groups.map(g => `<div class="card" style="margin-bottom:14px"><div class="card-head"><h3>${t('doc_' + g.c)}</h3><span class="badge b-neutral right">${g.docs.length}</span></div><div class="list">${g.docs.map(d => `<div class="list-item"><div class="ic blue" style="width:38px;height:38px;border-radius:10px;display:grid;place-items:center">${icon('folder')}</div><div class="grow"><b>${esc(d.name)}</b><div class="tiny subtle">${fmtDate(d.date)}${d.note ? ' · ' + esc(d.note) : ''}</div></div>${d.url ? `<a class="btn btn-secondary btn-sm" href="${esc(d.url)}" target="_blank" rel="noopener">${t('open')} ↗</a>` : `<span class="tiny subtle">${t('no_link')}</span>`}${canManage() ? `<button class="btn btn-ghost btn-sm btn-icon" onclick="openDocModal('${d.id}')">${icon('edit')}</button><button class="btn btn-ghost btn-sm btn-icon" onclick="confirmDialog('',()=>{DB.documents=DB.documents.filter(x=>x.id!=='${d.id}');saveData();render()})">${icon('trash')}</button>` : ''}</div>`).join('')}</div></div>`).join('') : `<div class="card">${empty('folder')}</div>`);
};
...
function openUnitDetail(id) {
  ...
  openModal({ title: `${unitLabel(u)} — ${esc(u.owner)}`, wide: true, body: `<div class="kpis" style="grid-template-columns:repeat(3,1fr)">${kpi('wallet', bal < 0 ? 'red' : 'green', t('balance'), `<span style="color:${bal < 0 ? 'var(--bad)' : 'var(--good)'}">${fmtMoney(bal, { noDual: true })}</span>`)}${kpi('home', 'blue', t('monthly_fee'), fmtMoney(u.fee, { noDual: true }))}${kpi('inbox', 'amber', t('nav_requests'), reqs)}</div>
    ...
    ${isAdmin() ? `<div class="label">${t('access')}</div><div class="card" style="box-shadow:none;margin-bottom:16px"><div class="card-body stack">
      ...
      ${login ? `<div class="row wrap" style="align-items:center;gap:10px"><span class="small muted grow">${t('manager_role_hint')}</span><label class="check" style="margin:0"><input type="checkbox" ${login.isManager ? 'checked' : ''} onchange="setUnitManager('${id}','${login.id}',this.checked)">${t('role_manager')}</label></div>` : ''}
    </div></div>` : ''}
...
// Grants/revokes the Manager role — a normal table update, not an Edge Function
// call, since profiles' own RLS already restricts writes to admins only.
async function setUnitManager(unitId, userId, on) {
  const { error } = await sb.from('profiles').update({ is_manager: on }).eq('id', userId);
